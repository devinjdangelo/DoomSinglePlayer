...
class DoomSimulator:
	...
	def train(self,size,update_n,start_step=0,save_n=25,test_n=25,init_explore=None,init_reward=None,
		...
		while True:
			...
			comm.Barrier()
			if episode % (update_n//n_workers)==0 and self.experience_memory.n_episodes>=(size//n_workers):
				iterations = (self.train_epochs * update_n) // size + 1
				for i in range(1,iterations+1):
					f = 1 - (i-1)/iterations
					lr = self.lr_schedule(f)
					clip = self.clip_schedule(f)
					batch = self.experience_memory.get_batch(size//n_workers)
					ploss,closs,entropy,g_n=self.agent.update(size//n_workers,batch,training_steps,lr,clip)
					if rank==0:
